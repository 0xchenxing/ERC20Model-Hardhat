...
import "./SecurityPool.sol";

contract BaseToken is ERC20, ERC20Burnable, Ownable, ReentrancyGuard, Pausable {
    // 总供应量，假设为10亿代币，18位小数
    ...
    uint256 public constant YEAR = 365 days;     // 年

    // 各分配的默认释放参数
    uint256 public constant SEED_LOCK_PERIOD = 12 * 30 * DAY;          // 种子锁仓12个月
    uint256 public constant SEED_RELEASE_PERIOD = 24 * 30 * DAY;       // 种子线性释放24个月
    uint256 public constant STARTUP_RELEASE_PERIOD = 6 * 30 * DAY;     // 初创激励6个月内线性释放
    uint256 public constant STRATEGIC_ROUND_LOCK_PERIOD = 6 * 30 * DAY; // 战略轮锁仓6个月
    uint256 public constant STRATEGIC_ROUND_RELEASE_PERIOD = 18 * 30 * DAY; // 战略轮线性释放18个月
    uint256 public constant STRATEGIC_LOCK_PERIOD = 12 * 30 * DAY;     // 战略储备锁仓12个月
    uint256 public constant STRATEGIC_RELEASE_PERIOD = 36 * 30 * DAY;  // 战略储备线性释放36个月
    uint256 public constant ECOSYSTEM_BURN_AFTER = 4 * 365 * DAY;        // 生态激励4年后销毁剩余
    uint256 public constant PROJECT_DEFAULT_LOCK_PERIOD = 12 * 30 * DAY; // 项目方团队锁仓12个月
    uint256 public constant PROJECT_DEFAULT_RELEASE_PERIOD = 36 * 30 * DAY; // 项目方团队线性释放36个月
    
    // 生态激励年度释放上限百分比
    uint256 public constant ECOSYSTEM_YEAR1_RATE = 40; // 第一年40%
    uint256 public constant ECOSYSTEM_YEAR2_RATE = 30; // 第二年30%
    uint256 public constant ECOSYSTEM_YEAR3_RATE = 20; // 第三年20%
    uint256 public constant ECOSYSTEM_YEAR4_RATE = 10; // 第四年10%

    // 安全资金池与紧急阈值
    SecurityPool public immutable securityPool;
    uint256 public emergencyThreshold = 1 ether; // 默认阈值（可由所有者调整）
    ...
    // 生态激励年度释放跟踪
    uint256 public ecosystemYear1Released; // 第一年已释放金额
    uint256 public ecosystemYear2Released; // 第二年已释放金额
    uint256 public ecosystemYear3Released; // 第三年已释放金额
    uint256 public ecosystemYear4Released; // 第四年已释放金额
    ...
    event SecurityPoolConfigured(address indexed pool, uint256 migratedAmount);
    event EmergencyThresholdUpdated(uint256 newThreshold);
    event EmergencyPauseTriggered(address indexed caller, uint256 observedValue);
    ...
    constructor(address projectOwner) ERC20("BaseToken", "BASE") Ownable(projectOwner) {
        ...
        // 部署并初始化安全资金池，托管剩余代币
        SecurityPool pool = new SecurityPool(IERC20(address(this)), projectOwner, address(this));
        securityPool = pool;

        uint256 remaining = balanceOf(address(this));
        if (remaining > 0) {
            _transfer(address(this), address(pool), remaining);
        }
        emit SecurityPoolConfigured(address(pool), remaining);
    }
    ...
    // 更新紧急阈值
    function updateEmergencyThreshold(uint256 newThreshold) external onlyOwner {
    ...
    // 触发紧急暂停
    function triggerEmergencyPause(uint256 observedValue) external {
        require(msg.sender == owner() || msg.sender == oracleAddress, "Not authorized");
        require(observedValue >= emergencyThreshold, "Below threshold");
        if (!paused()) {
            _pause();
            securityPool.controllerPause();
            emit EmergencyPauseTriggered(msg.sender, observedValue);
        }
    }
    ...
    function resumeFromEmergency() external onlyOwner {
        if (paused()) {
            _unpause();
            securityPool.controllerUnpause();
        }
    }
    ...
    function addVesting(string calldata category, address beneficiary, uint256 amount) external onlyOwner {
        ...
        require(_isSupportedVestingCategory(category), "Invalid category");
    ...
    function claimAllVesting() external nonReentrant {
        ...
        _payoutFromSecurityPool(msg.sender, totalReleasable);
    }
    ...
    function claimUnassignedSeed() external onlyOwner {
        ...
        _payoutFromSecurityPool(owner(), amount);
    ...
    function claimAirdrop() external nonReentrant {
        ...
        airdropPool -= amount;
        _payoutFromSecurityPool(msg.sender, amount);
    ...
    function _handleMultiLevelReferral(uint256 reward, address user) internal {
        ...
        for (uint256 i = 0; i < MAX_REFERRAL_LEVELS && current != address(0); i++) {
            ...
            // 检查是否在年度释放限额内
            if (_checkEcosystemReleaseLimit(refReward)) {
                ecosystemPool -= refReward;
                _payoutFromSecurityPool(current, refReward);
    ...
    // 检查生态激励年度释放限额
    function _checkEcosystemReleaseLimit(uint256 amount) internal returns (bool) {
        uint256 elapsedTime = block.timestamp - deployTime;
        
        // 计算当前所属年度
        uint256 currentYear = 1;
        if (elapsedTime >= 1 * YEAR) currentYear = 2;
        if (elapsedTime >= 2 * YEAR) currentYear = 3;
        if (elapsedTime >= 3 * YEAR) currentYear = 4;
        if (elapsedTime >= 4 * YEAR) return false; // 四年后停止释放
        
        // 计算年度释放限额
        uint256 yearLimit = 0;
        if (currentYear == 1) {
            yearLimit = ECOSYSTEM_INCENTIVE_ALLOCATION * ECOSYSTEM_YEAR1_RATE / 100;
            if (ecosystemYear1Released + amount <= yearLimit) {
                ecosystemYear1Released += amount;
                return true;
            }
        } else if (currentYear == 2) {
            yearLimit = ECOSYSTEM_INCENTIVE_ALLOCATION * ECOSYSTEM_YEAR2_RATE / 100;
            if (ecosystemYear2Released + amount <= yearLimit) {
                ecosystemYear2Released += amount;
                return true;
            }
        } else if (currentYear == 3) {
            yearLimit = ECOSYSTEM_INCENTIVE_ALLOCATION * ECOSYSTEM_YEAR3_RATE / 100;
            if (ecosystemYear3Released + amount <= yearLimit) {
                ecosystemYear3Released += amount;
                return true;
            }
        } else if (currentYear == 4) {
            yearLimit = ECOSYSTEM_INCENTIVE_ALLOCATION * ECOSYSTEM_YEAR4_RATE / 100;
            if (ecosystemYear4Released + amount <= yearLimit) {
                ecosystemYear4Released += amount;
                return true;
            }
        }
    ...
    function unstake(uint256 index) external nonReentrant {
        ...
        if (reward > 0) {
            ...
            // 检查生态激励年度释放限额
            if (_checkEcosystemReleaseLimit(reward)) {
                ecosystemPool -= reward;
                _payoutFromSecurityPool(msg.sender, reward);
    ...
    function claimAllRewards() external nonReentrant {
        ...
        // 检查生态激励年度释放限额
        if (_checkEcosystemReleaseLimit(totalReward)) {
            ecosystemPool -= totalReward;
            _payoutFromSecurityPool(msg.sender, totalReward);
            ...
            revert("Exceeds ecosystem release limit");
        }
    }
    ...
    function _isSupportedVestingCategory(string memory category) internal view returns (bool) {
        for (uint256 i = 0; i < vestingCategories.length; i++) {
            if (keccak256(bytes(vestingCategories[i])) == keccak256(bytes(category))) {
                return true;
            }
        }
        return false;
    ...
    function _payoutFromSecurityPool(address recipient, uint256 amount) internal {
        if (amount == 0) return;
        if (address(securityPool) != address(0)) {
            securityPool.withdraw(recipient, amount);
        } else {
            _transfer(address(this), recipient, amount);
        }
    }

    function _burnFromSecurityPool(uint256 amount) internal {
        if (amount == 0) return;
        if (address(securityPool) != address(0)) {
            securityPool.withdraw(address(this), amount);
        }
        _burn(address(this), amount);
    }

    function _storeInSecurityPool(uint256 amount) internal {
        if (amount == 0 || address(securityPool) == address(0)) return;
        _transfer(address(this), address(securityPool), amount);
    }
